...
# df = pd.read_csv("azure_global_demand_dirty_3yr.csv")
...
# df["usage_units"] = df.groupby(["region","service_type"])["usage_units"].transform(lambda x: x.interpolate())
# df["availability_pct"] = df.groupby(["region","service_type"])["availability_pct"].transform(lambda x: x.interpolate())
